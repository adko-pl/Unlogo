...
static av_cold int init(AVFilterContext *ctx, const char *args, void *opaque)
{
	...
	if ( pos != NULL ) {
		//av_log(ctx, AV_LOG_ERROR, "invalid input parameter: %s\n", args);
		//return -1;
		*pos++ = 0;	// terminate the string after the :
	}
	
	av_log(ctx, AV_LOG_DEBUG, "plugin: %s\n", plugin_name  );
    
	//av_log(ctx, AV_LOG_DEBUG, "plugin_name: %s\n", pos );
    
	plugin->handle = dlopen( plugin_name, RTLD_NOW );
